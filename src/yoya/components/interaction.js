...
class VTree extends Tag {
  static _stateAttrs = ['checkable', 'multiple', 'expanded'];
  ...
  constructor(setup = null) {
    ...
    this.registerStateAttrs(...this.constructor._stateAttrs);
    this.initializeStates({ checkable: false, multiple: true, expanded: true });
    this._setupBaseStyles();
    this.saveBaseStylesSnapshot();
    this._registerStateHandlers();
  ...
  _registerStateHandlers() {
    this.registerStateHandler('checkable', (checkable, host) => {
    ...
    const treeEl = div(t => {
      ...
      const renderNodes = (nodes, level) => {
        nodes.forEach(node => {
          ...
          // 先渲染当前节点
          t.child(this._renderNodeEl(node, level));
      ...
      renderNodes(this._data, 0);
    });

    this._children.push(treeEl);
  }
...
class VTreeSelect extends Tag {
  ...
  _createDropdown() {
    this._dropdownEl = div(d => {
      d.addClass('yoya-tree-select__dropdown');
      d.styles({
        position: 'absolute',
        backgroundColor: '#fff',
        borderRadius: '4px',
  ...
  _handleSelect(node) {
    this._value = node.key;
    this._selectedLabel = node.title || node.label;
    this.setState('visible', false);

    if (this._triggerEl) {
      this._triggerEl.text(this._selectedLabel || this._placeholder);
    }
  ...
  _bindEvents() {
    ...
    document.addEventListener('click', (e) => {
      if (this._dropdownEl && this.hasState('visible')) {
        const dropdownEl = this._dropdownEl.renderDom();
        const triggerEl = this._triggerEl ? this._triggerEl.renderDom() : null;
        if (!dropdownEl.contains(e.target) && (triggerEl && !triggerEl.contains(e.target))) {
          this.setState('visible', false);
        }
      }
  ...
  _syncTargetEvents() {
    ...
    if (this._triggerEl) {
      // 确保目标元素已经渲染
      if (!this._triggerEl._el) {
        this._triggerEl.renderDom();
      }
      if (this._triggerEl._el) {
        this._triggerEl._applyEventsToEl();
      }
    }
  }
  ...
  placeholder(p) {
    this._placeholder = p;
    if (this._triggerEl && !this._selectedLabel) {
      this._triggerEl.text(p);
    }
    return this;
  }
