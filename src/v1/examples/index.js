...
import {
  flex, grid, responsiveGrid, vstack, hstack, card, cardHeader, cardBody,
  menu, menuItem, vButton, toast,
} from '../../yoya/index.js';
...
export function createHomePage() {
  return appLayout({
    ...
    content: (content) => {
      // 欢迎区域
      content.child(card(welcome => {
        welcome.styles({
          marginBottom: '32px',
          background: 'linear-gradient(135deg, rgba(102, 126, 234, 0.1) 0%, rgba(118, 75, 162, 0.1) 100%)',
        });

        welcome.cardHeader(h => {
          h.styles({ borderBottom: 'none', paddingBottom: '0' });
          h.child(menuItem('Yoya.Basic V1', h1 => {
            h1.styles({ fontSize: '28px', fontWeight: '700', color: 'var(--islands-text, #333)' });
          }));
        });

        welcome.cardBody(p => {
          p.child(menuItem('一个浏览器原生的 HTML DSL 库，提供类似 Kotlin HTML DSL 的声明式语法。使用纯 ES 模块，无需构建工具，开箱即用。', desc => {
      ...
      content.child(docSection('features', '核心特性', [
        responsiveGrid(g => {
          ...
          g.child(card(c => {
            c.cardHeader(h => { h.styles({ fontSize: '16px', fontWeight: '600' }); h.text('📦 开箱即用'); });
            c.cardBody(p => { p.styles({ fontSize: '14px', lineHeight: '1.6', color: 'var(--islands-text-secondary, #666)' }); p.text('纯 ES 模块实现，无需构建工具'); });
          }));

          g.child(card(c => {
            c.cardHeader(h => { h.styles({ fontSize: '16px', fontWeight: '600' }); h.text('🎨 主题系统'); });
            c.cardBody(p => { p.styles({ fontSize: '14px', lineHeight: '1.6', color: 'var(--islands-text-secondary, #666)' }); p.text('支持浅色/深色模式切换'); });
          }));

          g.child(card(c => {
            c.cardHeader(h => { h.styles({ fontSize: '16px', fontWeight: '600' }); h.text('⚡️ 流式 API'); });
            c.cardBody(p => { p.styles({ fontSize: '14px', lineHeight: '1.6', color: 'var(--islands-text-secondary, #666)' }); p.text('链式调用设计，开发体验流畅'); });
          }));

          g.child(card(c => {
            c.cardHeader(h => { h.styles({ fontSize: '16px', fontWeight: '600' }); h.text('🔧 状态机'); });
            c.cardBody(p => { p.styles({ fontSize: '14px', lineHeight: '1.6', color: 'var(--islands-text-secondary, #666)' }); p.text('内置状态机机制，管理复杂交互'); });
          }));
        }),
      ]));
